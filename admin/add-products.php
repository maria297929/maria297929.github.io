...
if (!empty($_POST)) {
    ...
    $date = isset($_POST['date_added']) ? $_POST['date_added'] : date('Y-m-d H:i:s');
    // Insert new record into the contacts table
    $location= "../img/";
    $location.= $img;
    $stmt = $pdo->prepare('INSERT INTO products VALUES (?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([$id, $name, $desc, $price, $location, $size, $date]);
...
<div class="home_content">
    ...
    <form action="add-products.php" method="post">
        ...
        <input type="text" name="price" placeholder="12.99" id="price">
</br>
        <label for="size">Size</label>
        <input type="text" name="size" placeholder="30x60" id="size">
</br>       
        <label for="size">Date</label>
        <input type="datetime-local" name="date" placeholder="7856522" value="<?=date('Y-m-d\TH:i')?>" id="date">
</br>
