...
$num_orders = $pdo->query('SELECT COUNT(*) FROM orders')->fetchColumn();
 foreach ($orders as $order): ?>
  <tr>
      ...
      <td><?=$order['code']?></td>
      <td class="actions">
          <a href="ordersprocess.php?id=<?=$order['id']?>" class="edit"><i class="fas fa-pen fa-xs"></i></a>
          <a href="order-products.php?timestamp=<?=$order['timestamp']?>" class="view"> <i class="fa fa-eye" aria-hidden="true"></i></a>
      </td>
  </tr>
  <?php endforeach; ?>
                  </tbody>
		...
		<?php if ($page > 1): ?>
		<a href="orders.php?page=<?=$page-1?>"><i class="fas fa-angle-double-left fa-sm"></i></a>
		<?php endif; ?>
		<?php if ($page*$records_per_page < $num_orders): ?>
		<a href="orders.php?page=<?=$page+1?>"><i class="fas fa-angle-double-right fa-sm"></i></a>
		<?php endif; ?>
	  </div>
                </div>
              </div>
